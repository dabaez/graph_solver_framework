...
class PuLPSolver:
    ...
    def solve(self, graph: nx.Graph) -> Solution:
        ...
        solver.timeLimit = 30
        prob.solve(solver)
        independent_set = MaximumIndependentSet(
            [
                str(node)
                for node in graph.nodes()
                if pulp.value(node_vars[str(node)]) == 1
            ]
        )
        return Solution(mis=independent_set, time=prob.solutionTime)
