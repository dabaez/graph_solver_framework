...
class RolloutStorage(object):
    def __init__(self, max_t, batch_size, num_samples):
        ...
        self.action_log_probs: torch.Tensor = torch.Tensor([])
    ...
    # Changed parameter 'g' to 'data' to reflect PyG conventions
    def insert_ob_and_data(self, ob, data):
        ob_shape = list(ob.size())
        num_nodes = ob_shape[0]
        num_samples = ob_shape[1]
        # Store the PyG Batch object instead of the DGL graph
        self.data = data
        self.obs = torch.zeros(self.max_t + 1, *ob_shape)
        self.actions = torch.zeros(self.max_t, num_nodes, num_samples, dtype=torch.long)
        self.action_log_probs = torch.zeros(self.max_t, num_nodes, num_samples)
        self.obs[0].copy_(ob)
        self.masks.fill_(0)
    ...
    def insert_tensors(self, ob, action, action_log_prob, value_pred, reward, done):
        # This method is purely tensor-based and requires no changes.
        ob_ = ob.cpu()
        action_ = action.cpu()
    ...
    def compute_rets_and_advantages(self, gamma):
        ...
        for t in reversed(range(self.max_t)):
            self.rets[t] = (
                self.rewards[t] + gamma * self.masks[t + 1] * self.rets[t + 1]
            )
    ...
    def build_update_sampler(self, optim_batch_size, optim_num_samples):
        # Use PyG properties to get graph information
        num_nodes = self.data.num_nodes
        batch_size = self.data.num_graphs

        # The rest of this method is tensor manipulation and remains unchanged.
        flat_obs = (
            self.obs.narrow(0, 0, self.step_t)
            .permute(0, 2, 1, 3)
            .reshape(-1, num_nodes, self.obs.size(3))
        )
        flat_actions = (
            self.actions.narrow(0, 0, self.step_t)
            .permute(0, 2, 1)
            .reshape(-1, num_nodes)
        )
        flat_action_log_probs = (
            self.action_log_probs.narrow(0, 0, self.step_t)
            .permute(0, 2, 1)
            .reshape(-1, num_nodes)
        )
        flat_value_preds = (
            self.value_preds.narrow(0, 0, self.step_t)
            .permute(0, 2, 1)
            .reshape(-1, batch_size)
        )
        flat_rets = (
            self.rets.narrow(0, 0, self.step_t).permute(0, 2, 1).reshape(-1, batch_size)
        )
        flat_advantages = (
            self.advantages.narrow(0, 0, self.step_t)
            .permute(0, 2, 1)
            .reshape(-1, batch_size)
        )
